...
class AuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            # 跳过健康检查和根路径的认证
            if path in ["/", "/health", "/docs", "/openapi.json", "/redoc", "/admin/auth/email/send-captcha", "/admin/auth/email/sign"]:
    ...
    async def _send_error_response(self, send, status_code: int, detail: str):
        ...
        await send({"type": "http.response.body", "body": f'{{"detail": "{detail}"}}'.encode("utf-8")})
