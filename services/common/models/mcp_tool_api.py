...
class McpToolApi(Base):
    ...
    url: Mapped[str] = mapped_column(String(512), nullable=False, comment="API request URL")
    ...
    headers: Mapped[dict] = mapped_column(JSON, nullable=True, comment="Request headers (JSON format)")
    params: Mapped[str] = mapped_column(String, nullable=True, comment="Request parameters definition")
    request_body: Mapped[str] = mapped_column(String, nullable=True, comment="Request body content")
    request_schema: Mapped[dict] = mapped_column(JSON, nullable=True, comment="Request body schema (JSON Schema format)")
    response_schema: Mapped[dict] = mapped_column(JSON, nullable=True, comment="Response body schema (JSON Schema format)")
    request_demo: Mapped[str] = mapped_column(String, nullable=True, comment="Request example")
    response_demo: Mapped[str] = mapped_column(String, nullable=True, comment="Response example")
